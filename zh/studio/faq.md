...
**Q：误将 K3 镜像烧录到 K1 设备（或反之）后无法启动？**

K1 和 K3 系列设备使用不同的处理器架构和硬件配置，镜像不兼容。如果错误烧录了不匹配的镜像，设备将无法正常启动。

**解决方法：** 重新烧录与设备型号对应的正确镜像即可恢复正常。例如：
- K1 设备（MUSE Pi Pro、MUSE Pi 等）需使用 K1 系列镜像
- K3 设备（CoM260 Kit、Pico-ITX 等）需使用 K3 系列镜像
